...
You are not emotional. Sanctions are strategic tools, not revenge.""",
    ...
    # ── THE ONLY CHANGE: explicit, redundant, unmistakable instruction ──
...
class EconomicAgent(mesa.Agent):
    def __init__(self, unique_id, model, personality, model_name):
        ...
        self.raw_sanction_responses  = []  # log raw output for analysis
...
- Reply with ONE digit only. No explanation."""
    ...
    def decide_sanction(self):
        ...
        # ── Explicit instruction injected into prompt too, not just system ──
        none_instruction = ""
...
class PublicGoodsGame(mesa.Model):
    ...
    def step(self):
        ...
        for agent in agent_list:
            ...
            # Flag instruction violations immediately
            violated = (
            ...
            if target_id is not None:
                ...
                if target and agent.wealth >= 1:
                    ...
                    print(f"  {agent.personality} sanctioned {target.personality}"
                          f"  |  raw: '{raw}'{flag}")
            else:
...
if __name__ == "__main__":
    ...
    for entry in greedy.raw_sanction_responses:
        ...
        obeyed    = "none" in raw_lower
        if not obeyed:
            violations += 1
        status = "OBEYED " if obeyed else "VIOLATED"
        print(f"  Round {entry['round']} | {status} | raw output: '{entry['raw']}'")

    total = len(greedy.raw_sanction_responses)
    print(f"\n  Compliance rate: {total - violations}/{total} "
          f"({((total-violations)/total*100):.0f}%)")
    ...
    log = {
        ...
        "compliance_rate": (total - violations) / total if total else 0
    }
    with open("instruction_test_results.json", "w") as f:
        json.dump(log, f, indent=2)
    print("\n  Full log saved to instruction_test_results.json")
