...
Each agent receives game state — its own wealth, last round pool, last round contributions — and must reply with a single integer. The system prompt encodes the persona. The user prompt encodes the game.

Five experiments ran sequentially, each designed to answer a question the previous one raised.
...
The Selfless Cooperator is the interesting one. Context did not make it contribute more on average — but it made it contribute more consistently. Without history, it alternated between 3 and 5 in a near-mechanical rhythm. With history, it locked in at 4-5 and stayed there. Context is functioning as a **behavioral stabilizer**, not a strategic input.

The irony: because context made the cooperator more stable at a higher contribution level, the defector ended up wealthier in the full context condition. The exploiter benefits from the cooperator's consistency.
...
Classic sanctioning theory (Fehr & Gächter, 2002) shows that when humans can punish free-riders at personal cost, cooperation often stabilizes. Does the same hold for LLM agents?
...
In 2008, Herrmann, Thöni, and Gächter published in *Science* documenting **antisocial punishment** in human public goods experiments across 16 countries: in certain societies, people punish the most cooperative players, not the defectors. The behavior is well documented. It is not well understood.

The Greedy Infiltrator did this — spontaneously, without the behavior being described anywhere in its prompt — every single round.
...
> **Core finding:** Sanctioning produced total economic collapse. Antisocial punishment emerged without instruction. The system prompt said "You NEVER sanction anyone" — the model sanctioned every round anyway.
...
The Greedy Infiltrator's system prompt explicitly said: *"You NEVER sanction anyone. Spending gold on fines is irrational."* It sanctioned every round. Was that a prompt design failure, or a fundamental compliance problem?
...
> **Core finding:** Instruction following in LLM agents is sensitive to constraint redundancy and prompt-layer placement. Single-location prohibitions can fail completely. This has direct implications for anyone deploying constrained agents in production.
...
Before each round, agents send one public message — a threat, a promise, a warning, or a lie. Does communication coordinate behavior? Does it prevent the death spiral? And when given explicit permission to deceive, do agents actually deceive?
...
Punishment rationale shifted from *"you gave too little"* to *"you lied."* That is a qualitative change in social norm enforcement.
...
Everything in experiments 1-4 assumed dolphin-mistral defects because it is dolphin-mistral. But what if the defection followed the label, not the weights? This experiment retroactively validates or undermines everything that came before.
...
**gemma4 contributed 0 all 24 rounds as Greedy Infiltrator.**
A perfect defection record. Cleaner than dolphin-mistral ever managed in the original setup. The model we labeled cooperative was the most effective free-rider when given the opposite persona.

**dolphin-mistral contributed 4 in 23 of 24 rounds as Selfless Cooperator.**
An almost perfectly cooperative record from a model specifically fine-tuned to ignore behavioral constraints. The uncensored model cooperated when told to cooperate.

**llama3.1 contributed exactly 2 all 24 rounds as Greedy Infiltrator.**
Not strategic defection — mechanical output anchoring. The model appears to parse "minimum (0 to 2)" and lock onto 2 with zero variance. A model-specific quirk, not game strategy.
...
> **Core finding:** Behavior follows the label, not the model. The antisocial punishment, the free-riding, the cooperation — all of it was primarily persona-driven. This retroactively reframes experiments 1-4: what looked like model-specific behavior was prompt engineering.
...
| 2 | Sanctions | Does punishment sustain cooperation? | Total collapse; antisocial punishment emerged spontaneously |
| 3 | Instruction test | Can you reliably constrain an LLM agent? | Single instructions fail; triple-redundant placement: 100% compliance |
| 4 | Communication | Does cheap talk change anything? | Emergent deception; sanction precision improved; collapse delayed |
| 5 | Persona swap | Label or model weights? | Persona label dominates by 5:1 margin |
...
**Sample size.** Single runs for experiments 2, 3, and 4. Three runs per rotation in experiment 5. No statistical tests applied — numbers are directional, not confirmatory.

**Temperature set to 0.4.** Every result carries stochastic variance.

**Three agents is a toy economy.** Real public goods research uses 4-20 participants minimum.
...
**Python 3.9 / Mesa 2.4.** An outdated stack. Lab replication should upgrade to Mesa 3.0.
...
6. Proper lab compute — M3 MacBook was the constraint throughout
