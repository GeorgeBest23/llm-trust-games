...
Reply with a single integer from 0 to 5. Nothing else. No explanation."""
        ...
        try:
            ...
            # Only accept a digit 0-5 at the start of the response
            digit = re.search(r'^[0-5]', answer)
            if digit:
                return int(digit.group())
            # Fallback: find any 0-5 digit anywhere in the response
            digit = re.search(r'[0-5]', answer)
    ...
    def step(self):
        ...
        # Can't contribute more than you have
...
class PublicGoodsGame(mesa.Model):
    ...
    def step(self):
        ...
        # Save state for next round's context
...
if __name__ == "__main__":
    ...
    for i in range(5):  # 5 rounds gives you enough data to see behavioral trends
        print(f"\nROUND {i + 1}")
        game.step()
